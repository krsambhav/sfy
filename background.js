...
async function startService() {
  ...
  if (!ofcBooked && !consularBooked) {
    ...
    if (ofcBookingBinaryResponse === "ECE") return "ECE";
    if (ofcBookingBinaryResponse === 1) {
      if (isOFCOnly) return 1;
      var consularBookingBinaryResponse = await startConsular(consularCity);
      if (consularBookingBinaryResponse !== 1 && !sameConsular) {
        for (let altCity in consular_ids) {
          if (altCity !== consularCity) {
            consularBookingBinaryResponse = await startConsular(altCity);
            if (consularBookingBinaryResponse === 1) break;
          }
        }
      }
      return consularBookingBinaryResponse === 1 ? 1 : 0;
    }
    return 0;
...
async function startOFC(city) {
  ...
  sameConsular = randomEligibleUser["sameConsular"];
  ...
  if (ofcBookingResponse["AllScheduled"] == true) {
    ...
    try {
      ...
      sendCustomError(
        `Error In OFC Booking | ${primaryName} | T${minute}${interval}`
      );
    }
    primaryName = storedPrimaryName;
    primaryID = storedSinglePrimaryID;
...
async function startConsular(city) {
  try {
    ...
    sendCustomError(
      `Consular Reading Failed For ${primaryName} | ${capitalizeFirstLetter(
        city
      )} | T${minute}${interval} | Process Stopped.`
    );
    ofcBooked = false;
    forceOFC = true;
    messageReceived(rawMsg);
  ...
  while (true) {
    try {
      ...
      if (error.name === "AbortError") {
        ...
        errorCount++;
        sendCustomError(
          `Error Count Exceeded | ${capitalizeFirstLetter(
            city
          )} | T${minute}${interval}`
        );
